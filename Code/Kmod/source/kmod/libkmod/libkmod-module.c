						...
						struct kmod_module **mod)
{
    ...
    //simple structute tht stores properties of a file
	err = stat(abspath, &st);
	if (err < 0) {
		err = -errno;
		DBG(ctx, "stat %s: %s\n", path, strerror(errno));
	...
	//check if same module already exists in the kmod modules list stored inside ctx(typically depmod->ctx)
	...
	if (m != NULL) {
		...
		//inctemet ref count of the module
		*mod = kmod_module_ref(m);
		return 0;
	}
...
KMOD_EXPORT int kmod_module_get_dependency_symbols(const struct kmod_module *mod, struct kmod_list **list)
{
	...
	//this is the guy fetching the elf from the list of module perhaps
	elf = kmod_module_get_elf(mod);

	//if module doiest exist then retyrn errno
	if (elf == NULL)
		return -errno;
