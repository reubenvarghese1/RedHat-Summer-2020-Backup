...
static int depmod_modules_search_file(struct depmod *depmod, size_t baselen, size_t namelen, const char *path)
{
	...
	//check if module comes intree and choose appropriate
...
static int depmod_modules_build_array(struct depmod *depmod)
{
	...
	while (hash_iter_next(&module_iter, NULL, &v)) {
		...
		mod->idx = depmod->modules.count;/* index in depmod->modules.array */
...
load_info:
		...
		//I would check to see if this is negative. but that just shows if dependency found or not, we also need to check crc
...
static int depmod_load_module_dependencies(struct depmod *depmod, struct mod *mod)
{
	...
	kmod_list_foreach(l, mod->dep_sym_list) {
		...
		//This as well should not have continue but rather should return an error.
		// How can we load a module whose dependency requirements have not been met
		if (sym == NULL) {
		...
		//Checks and adds anyway. I changed this such that if even a single dependency fails, this will return -1 to previous
		// calling function so that this module may be skipped. Because if even a single dependency requirement is not met
		// it could cause unexpected behaviour when trying to run the module that depends on this.
...
//Essentually loads dependencies of all modules one by one by calling the function above this one
static int depmod_load_dependencies(struct depmod *depmod)
{
	...
	for (; itr < itr_end; itr++) {
		...
		//This guy actually checks the crc etc
		if(depmod_load_module_dependencies(depmod, mod)==-1) {
		    DBG("Removing module %s as dependency loading failed", mod->path);
            depmod_remove_single_module(depmod, mod);//the logic for this is yet to be written. One complication is to
        }
	}
...
static int depmod_calculate_dependencies(struct depmod *depmod)
{
	...
	/* populate modules users (how many modules uses it) */// Also if indegree 0 then add to array. These are the roots
...
static int depmod_load_single_modules(struct depmod *depmod, struct mod *mod)
{
    ...
    DBG("loaded symbols for %s\n", mod->path);

    return 0;
}

static int depmod_determine_if_compatible(struct depmod *depmod, const char *modname){
    //initially on depmod search, it will find all modules including external ones. External needs to be modified slightly
    // try to find module
    struct mod *mod;
    mod = hash_find(depmod->modules_by_name, modname);
    //module not found during search
    if(mod==NULL){
        return -1;//this surely needs to be changed to return an errno or something but it should return an error since
...
{
	/* __this_module is magic inserted by kernel loader. */// When loading a module, thi_module will point to it
...
static int do_depmod(int argc, char *argv[])
{
	...
	//if next index is less than argument count
	if (optind < argc) {
	    //Check if kernel version number is passed else try to deduce it
	...
	//directory to kernel stored in cfg.dirname . Read function sprintf for more
	...
	//creatre new structure with kmod. This has refcount set to 1
	...
	//just initializes the structures for eac of the things in the depmod struct
	err = depmod_init(&depmod, &cfg, ctx);
	if (err < 0) {
		CRIT("depmod_init: %s\n", strerror(-err));
		goto depmod_init_failed;
	}
	ctx = NULL; /* owned by depmod */// Since within depmod init, ctx has been given to depmod
	...
	if (all) {
	    //loading depmod configs . also where external is set
		err = cfg_load(&cfg, config_paths);
		if (err < 0) {
			CRIT("could not load configuration files\n");
			goto cmdline_modules_failed;
		}
		//Essentially searches modules, checks if it is there and then adds to database. This is again what weak modules tries to fix
